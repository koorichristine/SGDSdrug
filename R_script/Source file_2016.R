...
options(scipen = 999)
summary(Bayesian.model.2016, pars=c("alpha", "beta", "sigma", "logit_rho"), probs=c(0.025, 0.975))$summary
summary(Bayesian.model.2016, pars=c("rr_alpha", "rr_beta", "sigma", "logit_rho"), probs=c(0.025, 0.975))$summary

# check rHAT < 1.05
# n_eff > 3000
# check chains for samples that diverged (should not exceed 10% of the total 180000)
d1 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[1]][, 'divergent__'] %>% sum() # chain 1
d2 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[2]][, 'divergent__'] %>% sum() # chain 2
d3 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[3]][, 'divergent__'] %>% sum() # chain 3
d4 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[4]][, 'divergent__'] %>% sum() # chain 4
d5 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[5]][, 'divergent__'] %>% sum() # chain 5
d6 <- get_sampler_params(Bayesian.model.2016, inc_warmup = F)[[6]][, 'divergent__'] %>% sum() # chain 6
...
alpha.exc.probs <- Bayesian.model.2016 %>% spread_draws(alpha) %>% 
  summarise(alpha=threshold.intercept(alpha)) %>%
  pull(alpha)

threshold.coefficients <- function(x){mean(x > 0.00)}
beta.exc.probs <- Bayesian.model.2016 %>% spread_draws(beta[i]) %>% 
...
results <- as.data.frame(summary(Bayesian.model.2016, pars = c("rr_alpha", "rr_beta"), probs = c(0.025, 0.975))$summary)
...
write.csv(results, file = "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/2016 global relative risk coefficients.csv", row.names = FALSE)

# spatial maps of relative risks
# show first 6 rows only instead of the full 47
head(summary(Bayesian.model.2016, pars=c("rr_mu"), probs=c(0.025, 0.975))$summary)

# extraction key posterior results for the generated quantities 
relativeRisk.results <- as.data.frame(summary(Bayesian.model.2016, pars=c("rr_mu"), probs=c(0.025, 0.975))$summary)
...
excProbrr <- Bayesian.model.2016 %>% spread_draws(rr_mu[i]) %>% 
  group_by(i) %>% summarise(rr_mu=threshold(rr_mu)) %>%
  pull(rr_mu)

# insert the exceedance values into the spatial data frame
spatial_data_2016$excProb <- excProbrr

# Align the results to the areas in shp
spatial_data_2016$rr <- relativeRisk.results[, "rr"]
spatial_data_2016$rrlower <- relativeRisk.results[, "rrlower"]
spatial_data_2016$rrupper <- relativeRisk.results[, "rrupper"]

# create categories to define if an area has significant increase or decrease in risk, or nothing all 
spatial_data_2016$Significance <- NA
spatial_data_2016$Significance[spatial_data_2016$rrlower<1 & spatial_data_2016$rrupper>1] <- 0    # NOT SIGNIFICANT
spatial_data_2016$Significance[spatial_data_2016$rrlower==1 | spatial_data_2016$rrupper==1] <- 0  # NOT SIGNIFICANT
spatial_data_2016$Significance[spatial_data_2016$rrlower>1 & spatial_data_2016$rrupper>1] <- 1    # SIGNIFICANT INCREASE
spatial_data_2016$Significance[spatial_data_2016$rrlower<1 & spatial_data_2016$rrupper<1] <- -1   # SIGNIFICANT DECREASE

summary(spatial_data_2016$rr)
hist(spatial_data_2016$rr)
table(spatial_data_2016$Significance)

colnames(spatial_data_2016)[21] <- "ExcProb_2016"
colnames(spatial_data_2016)[22] <- "Risk_2016"
colnames(spatial_data_2016)[24] <- "rrupper"
colnames(spatial_data_2016)[25] <- "Sign_2016"
spatial_data_2016$Drug_2016 <- paste(round(spatial_data_2016$Risk_2016,2), " (95% CrI:", round(as.numeric(spatial_data_2016$rrlower),2), ",", round(as.numeric(spatial_data_2016$rrupper),2), ")", sep = "")

# Export this spatial data out for ArcGIS Pro visualisation 
st_write(spatial_data_2016, 
         dsn = "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/2016_results/2016_results_shp.shp")
...
custom_labels_sf <- st_as_sf(custom_labels, coords = c("custom_x", "custom_y"), crs = st_crs(spatial_data_2016))
...
tmap_save(rr_map, "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/rr_2016.png", height = 12)

# map of significance regions
sg_map <- tm_shape(spatial_data_2016) + 
  tm_fill("Sign_2016", style = "cat", title = "Significance Categories", 
...
tmap_save(sg_map, "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/sign_2016.png", height = 12)
...
# Cut data into bins
spatial_data_2016$ProbCat <- cut(spatial_data_2016$ExcProb_2016, breaks = breaks, labels = labels, include.lowest = TRUE)

# Convert to factor for plotting
spatial_data_2016$ProbCat <- as.factor(spatial_data_2016$ProbCat)

# Map of exceedance probabilities
ep_map <-tm_shape(spatial_data_2016) + 
...
tmap_save(ep_map, "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/excprob_2016.png", height = 12)
...
risk_data <- st_drop_geometry(spatial_data_2016[,c("LAD21CD", "LAD21NM", "Risk_2016", "Sign_2016", "Drug_2016")])
# all results will be sinked into this INITIAL file
initial_file <- read.csv(file = "C:/Users/chris/Desktop/SGDS/Stan_Output_Results/2015.txt", sep = ",")
append_file <- merge(initial_file, risk_data, by.x=c("LAD21CD", "LAD21NM"), by.y=c("LAD21CD", "LAD21NM"))
# overwrite
write.csv(append_file, file="C:/Users/chris/Desktop/SGDS/Stan_Output_Results/2016.txt", row.names = FALSE, fileEncoding = "UTF-8")